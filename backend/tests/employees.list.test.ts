...
describe("GET /api/employees", () => {
    ...
    it("combines filters (country + department)", async () => {
        const res = await request(app).get(
            "/api/employees?country=India&department=Engineering"
        );
    ...
    it("searches by name", async () => {
        const res = await request(app).get("/api/employees?search=Employee 07");
    ...
    it("sorts by salary descending", async () => {
        const res = await request(app).get(
            "/api/employees?sortBy=salary&sortOrder=desc"
        );
    ...
    it("rejects an unknown sortBy with 422", async () => {
        const res = await request(app).get("/api/employees?sortBy=hacker");
        expect(res.status).toBe(422);
    });
});
...
describe("GET /api/employees/filter-options", () => {
    it("returns distinct sorted countries and departments", async () => {
        const res = await request(app).get("/api/employees/filter-options");
        expect(res.status).toBe(200);

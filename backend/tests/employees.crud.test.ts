...
describe("POST /api/employees", () => {
    ...
    it("returns 422 when required fields are missing", async () => {
        const res = await request(app).post("/api/employees").send({ name: "Only Name" });
        expect(res.status).toBe(422);
    });
});
...
describe("PUT /api/employees/:id", () => {
    ...
    it("returns 404 when updating a missing employee", async () => {
        const res = await request(app).put("/api/employees/99999").send({ salary: 100 });
        expect(res.status).toBe(404);
    });
...
describe("DELETE /api/employees/:id", () => {
    it("deletes and returns 204, then the employee is gone", async () => {
        const created = await request(app).post("/api/employees").send(validEmployee);
        const del = await request(app).delete(`/api/employees/${created.body.id}`);
        expect(del.status).toBe(204);

        const get = await request(app).get(`/api/employees/${created.body.id}`);
